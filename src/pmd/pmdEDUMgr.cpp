...
int pmdEDUMgr::returnEDU ( EDUID eduID, bool force, bool* destroyed )
{
        ...
        size() > (unsigned int)pmdGetKRCB()->getMaxPool () )
   {
      ...
      if ( destroyed )
      {
         ...
         // OK or MDB_SYS (edu can't be found), so that thread can terminate
         // itself
         if ( MDB_OK == rc || MDB_SYS == rc )
            *destroyed = true ;
         else
            *destroyed = false ;
      }
   ...
   else
   {
      ...
      // as long as we treat MDB_SYS as "destroyed" signal, we should be
      // safe here
...
int pmdEDUMgr::_createNewEDU ( EDU_TYPES type, void* arg, EDUID *eduid )
{
   ...
   MDB_VALIDATE_GOTOERROR ( cb, MDB_OOM,
