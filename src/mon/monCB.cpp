...
void MonAppCB::setClientCounter(long long clientCounter)
{
	_clientCounter = clientCounter;
}

long long MonAppCB::getClientCounter() const
{
	return _clientCounter;
}

void MonAppCB::increaseClientCounter()
{
	_mutex.get();
	_clientCounter++;
	_mutex.release();
...
void MonAppCB::decreaseClientCounter()
{
	_mutex.get();
	_clientCounter--;
	_mutex.release();
